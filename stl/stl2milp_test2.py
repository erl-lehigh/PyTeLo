...
# formula = "G[0,6](x <= 3) && G[0, 6] (y <= 4) && G[0,6](x >= 3) && G[0, 6] (y >= 1) && G[8,20](x >= 2) && G[8, 20] (y >= 2) && G[8,20](x <= 3) && G[8, 20] (y <= 2) && G[10,25] (z>=3) && G[10,25] (z<=3)"
...
stl_milp = stl2milp(ast, ranges={'x': [-4, 10], 'y': [-4, 10], 'z': [-4, 5]}, robust=True)
stl_milp.M = 20
...
for k in range(period):
    ...
    name = "u_{}".format(k)
    u[k] = stl_milp.model.addVar(vtype=grb.GRB.CONTINUOUS, lb=-3, ub=4, name=name)
    name = "v_{}".format(k)
...
stl_milp.model.addConstr(z[0] == 3)
# system constraints      
...
print(str(obj), ':', obj.getValue(), "hola")
