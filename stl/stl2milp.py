...
class stl2milp(object):
    ...
    def __init__(self, formula, ranges=None, vtypes=None, model=None,
        ...
        self.ranges = ranges
        if ranges is None:
            self.ranges = {v: (0, 1) for v in self.formula.variables()}
        ...
        def predicate(self, pred, z, t):
            ...
            if pred.relation in (RelOperation.GE, RelOperation.GT):
                self.model.addConstr(v - self.M * z <= self.rho)
                self.model.addConstr(v + self.M * (1 - z) >= self.rho)
